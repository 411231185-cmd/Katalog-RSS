...
src = 'RegTorg-Fis-MashPort/Пример-РегТорг.xls'
dst = 'RegTorg-Fis-MashPort/RegTorg_catalog_FINAL_20_unique.xml'

# Читаем пример каталога (эталон)
df = pd.read_excel(src)

# Оставляем только первые 20 строк с фото и уникальными артикулами
df = df[df['Фотография'].notnull() & (df['Фотография'] != '')].head(20).copy()

# Генерируем уникальные id и vendorCode (артикул)
