...
Источники:
  1. DIRECTUS_TKP_549_FULL.csv  — главный источник (фото, цена, описание)
...
Результат:
  СПИСОК ОФФЕРОВ+ОПИСАНИЕ_ДАТА.xlsx  — готов к импорту на promportal.su
"""
...
# ══════════════════════════════════════════════════════════════════
# КОНФИГ — пути к файлам
# ══════════════════════════════════════════════════════════════════
BASE = r"C:\GitHub-Repositories\Katalog-RSS"

SOURCES = {
    # Приоритет 1 — главный CSV (549 позиций с описаниями)
    "tilda_csv": os.path.join(BASE, "Directus-RSS", "Товары", "Каталоги CSV",
                              "DIRECTUS_TKP_549_FULL.csv"),

    # Приоритет 2 — каталог Олега
    "oleg_xlsx": os.path.join(BASE, "CATALOG_OLEG_FINAL_100.xlsx"),

    # Приоритет 3 — MD-файлы (имена + описания)
    "nomenclatura_md": os.path.join(BASE, "NOMENCLATURA RSS-2026.md"),
    "rzn_offers_md":   os.path.join(BASE, "PLOSIADKI-RSS", "PromPortal",
                                    "RZN-список офферов.md"),
    "parts_1m63_md":   os.path.join(BASE, "RZN-сайт",
                                    "Запасные части для токарно-винторезных станков "
                                    "мод. 1М63Н(1М63),16К40 (1А64),1Н65(1М65),РТ117,РТ817.md"),
}

# Уже загруженные на portale — исключаем дубли
EXCLUDE_FILE = os.path.join(BASE, "PLOSIADKI-RSS", "PromPortal", "СПИСОК ОФФЕРОВ.md")

# Результат — xlsx рядом с исключениями
OUTPUT_FILE = os.path.join(BASE, "PLOSIADKI-RSS", "PromPortal",
                           f"СПИСОК ОФФЕРОВ+ОПИСАНИЕ_{date.today().strftime('%Y%m%d')}.xlsx")
...
# ══════════════════════════════════════════════════════════════════
# АВТОГЕНЕРАЦИЯ ОПИСАНИЯ (если нет готового)
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# ПАРСЕР CSV — адаптирован под реальные колонки DIRECTUS_TKP_549_FULL.csv
# Колонки: SKU;Category;Title;Description;Text;Photo;Price;Quantity;...
# ══════════════════════════════════════════════════════════════════
def parse_tilda_csv(fp: str) -> list:
    if not os.path.exists(fp):
        print(f"  [SKIP] не найден: {fp}")
        return []
    ...
    df = pd.read_csv(fp, sep=sep, dtype=str, low_memory=False).fillna("")
    # Нормализуем заголовки: убираем пробелы, кавычки
    ...
    c_sku   = col("SKU", "Артикул", "sku")
    c_title = col("Title", "Наименование", "Название", "title")
    c_text  = col("Text", "Описание подробное", "text")
    c_desc  = col("Description", "Описание краткое", "description")
    c_photo = col("Photo", "Фото", "photo", "Изображение")
    c_price = col("Price", "Цена", "price")
    c_cat   = col("Category", "Категория", "category")
    # Совместимость может быть в Editions, Modifications или отдельном поле
    c_compat = col("Editions", "Modifications", "Совместимость", "Compatible")

    if not c_title:
        print(f"  [ERR] Не найдена колонка Title в {fp}")
        print(f"  Доступные колонки: {list(df.columns[:10])}")
        return []

    items = []
    for _, row in df.iterrows():
        title = str(row[c_title]).strip().strip('"')
        if not title or title.lower() in ("nan", "title", "наименование", "название"):
            continue
        ...
        # Совместимость: "1М65;1Н65;16К40" или "Параметр1:1М65;Параметр2:1Н65"
        compat = ""
        if c_compat:
            raw_compat = str(row[c_compat])
            # Извлекаем значения вида "Параметр1:1М65"
            vals = re.findall(r"Параметр\d+:([^;\"]+)", raw_compat)
            if vals:
                compat = ", ".join(v.strip() for v in vals)
            elif raw_compat not in ("nan", ""):
                compat = raw_compat.replace(";", ", ").strip()
...
def parse_oleg_xlsx(fp: str) -> list:
    ...
    if not c_title:
        print(f"  [WARN] CATALOG_OLEG_FINAL_100.xlsx: не найдена колонка Title")
        return []
    ...
    with open(fp, encoding="utf-8", errors="ignore") as f:
        for raw in f:
            ...
            if len(line) >= 120 and cur_title:
                cur_desc.append(clean_text(line))
                continue
...
def load_exclude(fp: str) -> set:
    if not os.path.exists(fp):
        print(f"  [WARN] файл исключений не найден: {fp}")
        return set()
    items = parse_md(fp)
...
def write_xlsx(items: list, out_path: str):
    ...
    print(f"  📦 Строк товаров: {len(items)}")
...
def main():
    ...
    print("\n[1/4] Загружаю список исключений (уже на портале)...")
    exclude = load_exclude(EXCLUDE_FILE)

    print("\n[2/4] Читаю источники (приоритет: CSV > XLSX > MD)...")
    sources = [
        parse_tilda_csv(SOURCES["tilda_csv"]),    # приоритет 1
        parse_oleg_xlsx(SOURCES["oleg_xlsx"]),     # приоритет 2
        parse_md(SOURCES["nomenclatura_md"]),      # приоритет 3
        parse_md(SOURCES["rzn_offers_md"]),        # приоритет 4
        parse_md(SOURCES["parts_1m63_md"]),        # приоритет 5
    ]
    ...
    if not merged:
        print("\n⚠️  Нет новых позиций для загрузки!")
        sys.exit(0)

    print("\n[4/4] Генерирую XLSX для импорта на promportal.su...")
