...
SOURCES = {
    # Приоритет 1 — самый богатый источник (Tilda): SKU, цена, фото, описание, совместимость
    "tilda_csv": os.path.join(BASE, "Directus-RSS", "Товары", "Каталоги CSV",
                              "DIRECTUS_TKP_549_FULL.csv"),

    # Приоритет 2 — каталог Олега: Title, SKU, Category_Path, Price, Compatible_SKUs, Full_Description
    "oleg_xlsx": os.path.join(BASE, "CATALOG_OLEG_FINAL_100.xlsx"),

    # Приоритет 3 — MD-файлы (имена без описаний, добавляем то чего нет в 1-2)
...
OUTPUT_FILE  = os.path.join(BASE, "PLOSIADKI-RSS", "PromPortal",
                            f"PROMPORTAL_READY_{date.today().strftime('%Y%m%d')}.xlsx")
...
def main():
    ...
    sources = [
        ...
        parse_oleg_xlsx(SOURCES["oleg_xlsx"]),         # второй источник
